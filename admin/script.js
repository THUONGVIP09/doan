...
function spin() {
  ...
  if (MagicRoaster.includes(Hasil[0])) SelectedItem = "Voucher giảm 20%";
  if (Sepeda.includes(Hasil[0])) SelectedItem = "1 áo du lịch";
  if (RiceCooker.includes(Hasil[0])) SelectedItem = "1 balo du lịch";
  if (LunchBox.includes(Hasil[0])) SelectedItem = "Voucher giảm 20%";
  if (Sanken.includes(Hasil[0])) SelectedItem = "Voucher 20% vé Asian-park";
  if (Electrolux.includes(Hasil[0])) SelectedItem = "Voucher giảm 10%";
  if (JblSpeaker.includes(Hasil[0])) SelectedItem = "Giảm 5% đặt Hotel";
  ...
  setTimeout(function () {
    ...
    swal(
      "Congratulations",
      "Phần thưởng của bạn là " + SelectedItem + ".",
